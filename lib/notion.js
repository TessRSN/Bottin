...
const PROP = {
  ...
  // 'Expertise' (ancien formulaire) : fusionne dans "Thèmes d'intérêt" le 2026-09-14,
  // propriete Notion vide depuis, a supprimer dans Notion (cf. BACKLOG).
  themes: "Thèmes de recherche ou d'intérêt", // rich_text — renommee dans Notion le 2026-09-15 (ex-« Thèmes d'intérêt »)
  axes: "Axes d'intérêt",     // multi_select
  principes: 'Principes fondateurs', // multi_select
  champs: "Champs d'action",  // multi_select
  projet: 'Présentation',        // rich_text — renommee dans Notion le 2026-09-15 (ex-« Projet de recherche »)
  ...
  photo: 'Photo',                                   // files (Phase 3a, 2026-09-14) — photo de profil
  // Phase 3d (2026-09-14): fiche auteur OpenAlex choisie par la personne
  // (« A5066925674 ») pour suggerer des themes depuis ses publications sans
  // ORCID. Technique : jamais affichee ni exportee dans le bottin.
  openalex: 'OpenAlex ID',                          // rich_text
  // Campagne « complétez votre profil » (2026-09-15) : date d'envoi du courriel
  // (vide = pas encore contactee). Permet le rythme quotidien et les relances.
  campagneProfil: 'Courriel campagne profil',       // date
  // Publication directe (2026-09-25) : une modification par le membre ne change
  // plus le statut workflow (plus de « Modifié » ni de revue) ; elle est horodatee
  // ici pour rester visible dans Notion (vue triee sur cette date).
  profilModifie: 'Profil modifié le',               // date
};

// Phase 3d : accepte l'identifiant nu ou l'URL https://openalex.org/A…,
// renvoie « A… » en majuscules, ou '' si le format n'est pas celui d'un auteur.
function normalizeOpenAlexId(value) {
  const m = String(value || '').trim().match(/^(?:https?:\/\/openalex\.org\/(?:authors\/)?)?(A\d{4,12})$/i);
...
const CSV_COL = {
  ...
  reseau: 'Réseau 1',
  themes: "Thèmes de recherche ou d'intérêt",
  ...
  photo: 'Photo', // adresse du relais /api/photo?id=...&v=... (vide si pas de photo)
};
...
// Propriete "files" -> { url, name, expiry } du 1er fichier, ou null.
// Les fichiers heberges par Notion ont une url temporaire (1 h) ; les
// fichiers "external" une url permanente.
function photoFromProp(prop) {
  const f = prop && Array.isArray(prop.files) && prop.files[0];
  if (!f) return null;
  if (f.type === 'file' && f.file) return { url: f.file.url, name: f.name || '', expiry: f.file.expiry_time || null };
  if (f.type === 'external' && f.external) return { url: f.external.url, name: f.name || '', expiry: null };
  return null;
}

// Appel brut a l'API Notion (endpoints que le SDK 2.2 ne connait pas,
// comme le televersement de fichiers).
const NOTION_VERSION = '2022-06-28';
async function notionRaw(path, init) {
  const headers = Object.assign({
    'Authorization': 'Bearer ' + process.env.NOTION_KEY,
    'Notion-Version': NOTION_VERSION,
  }, (init && init.headers) || {});
  const resp = await fetch('https://api.notion.com/v1' + path, Object.assign({}, init, { headers }));
  const json = await resp.json().catch(() => ({}));
  if (!resp.ok) {
    const e = new Error(`Notion ${path} -> ${resp.status} ${json.code || ''} ${json.message || ''}`.trim());
...
async function getProfile(pageId) {
  ...
  return {
    ...
    // Phase 3d: fiche auteur OpenAlex memorisee (vide si jamais choisie)
    openalex: getText(p[PROP.openalex]),
    ...
    // Phase 3a: photo de profil. photoUrl est temporaire (1 h), suffisant
    // pour l'apercu dans la page Mettre a jour mon profil.
    photoUrl: (photoFromProp(p[PROP.photo]) || {}).url || null,
    photoName: (photoFromProp(p[PROP.photo]) || {}).name || '',
  };
}

/**
 * Phase 3a (2026-09-14): photo de profil.
 * Televerse une image dans Notion (API File Upload, single_part) et
 * l'attache a la propriete "Photo" du membre (remplace l'ancienne).
 */
async function uploadMemberPhoto(pageId, buffer, contentType, filename) {
  const upload = await notionRaw('/file_uploads', {
    method: 'POST',
  ...
  await notionRaw(`/file_uploads/${upload.id}/send`, { method: 'POST', body: form });
  await notion().pages.update({
    page_id: pageId,
    properties: {
      [PROP.photo]: { files: [{ type: 'file_upload', file_upload: { id: upload.id }, name: filename }] },
    },
  });
  return { id: upload.id, name: filename };
}

async function removeMemberPhoto(pageId) {
  await notion().pages.update({ page_id: pageId, properties: { [PROP.photo]: { files: [] } } });
}

/** Pour le relais /api/photo : url temporaire + etat du membre (approuve ? consentant ?) */
async function getMemberPhoto(pageId) {
  const page = await notion().pages.retrieve({ page_id: pageId });
  const p = page.properties;
  return {
    photo: page.archived ? null : photoFromProp(p[PROP.photo]),
    workflow: getText(p[PROP.workflow]),
    consent: getText(p[PROP.consent]),
  };
}
...
async function updateProfile(pageId, data) {
  ...
  // Phase 3d: fiche OpenAlex choisie (null/'' = retiree). Format verifie ici.
  if (data.openalex !== undefined) properties[PROP.openalex] = { rich_text: richText(normalizeOpenAlexId(data.openalex)) };

  if (data.consent !== undefined) properties[PROP.consent] = { select: data.consent ? { name: data.consent } : null };

  // Phase 2i (2026-05-04): champ evaluateur ajoute au formulaire (questions
  // Q16/Q17 reintegrees depuis l'ancien Microsoft Forms). Select strict
  // Oui/Non/Incertain (cf. EVALUATEUR_OPTIONS dans api/membership-report.js).
  if (data.evaluateur !== undefined) properties[PROP.evaluateur] = { select: data.evaluateur ? { name: data.evaluateur } : null };

  // Publication directe (2026-09-25) : le statut workflow n'est plus touche.
  // Une fiche « Approuvé » reste publiee et la modification est visible au
  // prochain chargement ; une fiche « Nouveau » ou « Refusé » garde son statut.
  // Seule la date de modification par le membre est enregistree.
  properties[PROP.profilModifie] = { date: { start: new Date().toISOString() } };
  ...
  do {
    ...
    for (const page of resp.results) {
      ...
      members.push({
        ...
        orcid: getText(p[PROP.orcid]),
        cv: getText(p[PROP.cv]),
        openalex: getText(p[PROP.openalex]), // Phase 3d (sauvegardes ; pas dans le CSV public)
        evaluateur: getText(p[PROP.evaluateur]),
        consent: getText(p[PROP.consent]),
        workflow: getText(p[PROP.workflow]),
        ...
        // Phase 3a: nom horodate du fichier photo ('' si aucune) — sert de version
        photoName: (photoFromProp(p[PROP.photo]) || {}).name || '',
        campagneProfil: (p[PROP.campagneProfil] && p[PROP.campagneProfil].date && p[PROP.campagneProfil].date.start) || null,
        profilModifie: (p[PROP.profilModifie] && p[PROP.profilModifie].date && p[PROP.profilModifie].date.start) || null,
      });
    }
...
async function createMember(data) {
  ...
  // Phase 3d: fiche auteur OpenAlex choisie a l'inscription (facultatif)
  const openalexId = normalizeOpenAlexId(data.openalex);
  if (openalexId) properties[PROP.openalex] = { rich_text: richText(openalexId) };
  ...
  // Phase 2i (2026-05-04): nouveau champ evaluateur au formulaire (Oui/Non/Incertain)
  if (data.evaluateur) properties[PROP.evaluateur] = { select: { name: data.evaluateur } };
...
/**
 * Ecrit une propriete de type date (ISO AAAA-MM-JJ, ou null pour vider),
 * sans toucher au workflow. Utilise par la campagne de profil.
 */
async function setMemberDate(pageId, propKey, iso) {
  const name = PROP[propKey];
  if (!name) throw new Error('Propriete inconnue : ' + propKey);
  await notion().pages.update({ page_id: pageId, properties: { [name]: { date: iso ? { start: iso } : null } } });
}

module.exports = {
  PROP, CSV_COL, INST_PROP,
  findByEmail, getProfile, updateProfile, getAllMembers, createMember,
  markAcceptanceEmailSent, setMembershipDates, setMemberCheckbox, setMemberDate,
  ...
  uploadMemberPhoto, removeMemberPhoto, getMemberPhoto,
};
