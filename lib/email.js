 ...
 * - sendRenewalReminder60j / 30j / Confirmation / ArchiveNotification : cycle renouvellement
 * - sendAdminRetentionRecap: récap admin des fiches à archiver dans 7 jours
 * - sendEmailChangeConfirmation: confirmation changement courriel principal
 *
 * Tous les emails partent de bottin@rsn.quebec avec Reply-To: rsn.gestion@rimuhc.ca
 * pour rediriger les réponses vers l'équipe RSN sans passer par la boîte technique.
 *
 * MODE TEST : si EMAIL_TEST_MODE=true et EMAIL_TEST_RECIPIENT défini, TOUS les
 * envois sont redirigés vers EMAIL_TEST_RECIPIENT avec un sujet préfixé
 * "[TEST → original@email.com]" — utile pour tester en preview Vercel sans
 * spammer les vrais membres.
 *
 * Migré depuis Gmail SMTP / Nodemailer le 2026-05-15.
 */
const { Resend } = require('resend');

const FROM_NAME = process.env.RESEND_FROM_NAME || 'RSN — Bottin';
const FROM_EMAIL = process.env.RESEND_FROM_EMAIL || 'bottin@rsn.quebec';
const REPLY_TO = 'rsn.gestion@rimuhc.ca';
const CONTACT_EMAIL = 'rsn.gestion@rimuhc.ca';
const BOTTIN_URL = process.env.BASE_URL || 'https://bottin.rsn.quebec';
...
/**
 * Envoi unifié via Resend, avec gestion du mode test et du Reply-To.
 *
 * @param {object} opts
 * @param {string|string[]} opts.to - destinataire(s) — string ou array d'emails
 * @param {string} opts.subject - sujet du courriel
 * @param {string} opts.html - corps HTML (sans <html>, sans <body>)
 * @param {string|null} [opts.replyTo] - Reply-To (défaut: REPLY_TO ; null pour omettre)
 * @returns {Promise<object>} - réponse Resend ou { skipped: true } en mode test sans recipient
 */
async function _send({ to, subject, html, replyTo }) {
  // Mode test : on redirige TOUS les envois vers TEST_RECIPIENT et on
  // préfixe le sujet pour qu'on voie immédiatement le destinataire d'origine.
  let actualTo = to;
  let actualSubject = subject;
  if (TEST_MODE) {
    if (!TEST_RECIPIENT) {
      console.warn('[email] EMAIL_TEST_MODE=true mais EMAIL_TEST_RECIPIENT non défini — envoi annulé');
      return { skipped: true, reason: 'test_mode_no_recipient' };
    }
    const originalTo = Array.isArray(to) ? to.join(', ') : to;
    actualTo = TEST_RECIPIENT;
    actualSubject = `[TEST → ${originalTo}] ${subject}`;
    console.log(`[email] TEST_MODE: ${originalTo} → ${TEST_RECIPIENT} | ${subject}`);
  }
  ...
  // Le SDK Resend (resend.emails.send) attend replyTo en camelCase.
  // reply_to (snake_case) est ignoré silencieusement -> Reply-To absent.
  // null = omettre explicitement (sendAdminRetentionRecap notification interne).
  const effectiveReplyTo = replyTo === null ? null : (replyTo || REPLY_TO);
...
async function sendMagicLink(to, name, magicUrl, lang = 'fr') {
  ...
  await _send({ to, subject, html: emailWrapper(inner) });
}
...
// Formulaire d'abonnement hébergé Mailchimp (même audience que rsn.quebec). URL publique, pas un secret.
const NEWSLETTER_SIGNUP_URL = 'https://quebec.us22.list-manage.com/subscribe?u=d3aa458695db41d98223d8c36&id=5343fbdd97';
...
async function sendAcceptanceEmail(to, name, type) {
  ...
  const inner = `
    ...
    <p>En tant que membre, vous recevrez les courriels liés à votre adhésion (renouvellement, modifications de profil). Notre infolettre, elle, est facultative : actualités du réseau, appels à communication et événements à venir. Si ce n'est pas déjà fait, abonnez-vous en un clic. Vous pourrez vous désabonner à tout moment.</p>
    <div style="text-align:center;margin:1rem 0 1.5rem">
      <a href="${NEWSLETTER_SIGNUP_URL}" style="${btnStyle}">S'abonner à l'infolettre</a>
    </div>
    ...
    <h3 style="color:#2b6cb0;margin-top:2rem;margin-bottom:0.5rem;font-size:1rem">Communication</h3>
    <p>As a member, you will receive the emails related to your membership (renewal, profile changes). Our newsletter is optional: network news, calls for abstracts and upcoming events. If you haven't already, subscribe in one click. You can unsubscribe at any time.</p>
    <div style="text-align:center;margin:1rem 0 1.5rem">
      <a href="${NEWSLETTER_SIGNUP_URL}" style="${btnStyle}">Subscribe to the newsletter</a>
    </div>
...
async function sendAdminRetentionRecap(recipients, upcomingList) {
  ...
  // Admin recap : pas de Reply-To (notification interne, pas besoin de répondre).
  await _send({ to: recipients, subject, html: emailWrapper(inner), replyTo: null });
}
...
async function sendEmailChangeConfirmation(toNewEmail, name, confirmUrl, oldEmail, lang) {
  ...
  await _send({ to: toNewEmail, subject, html: emailWrapper(inner) });
}


/**
 * Campagne « complétez votre profil » (2026-09-15). Texte valide par Tess,
 * FR puis EN, GIF et capture heberges sur le site (img/). Le lien de
 * modification est valable 30 jours (signCampaignToken).
 */
async function sendProfileCampaign(to, name, magicUrl) {
  const baseUrl = process.env.BASE_URL || 'https://bottin.rsn.quebec';
  const subject = 'Le bottin du RSN a changé : venez voir votre profil / The RSN directory has changed: come see your profile';
  const inner = `
  <!-- ───────────── FRANÇAIS ───────────── -->
  <p><strong>Bonjour ${name},</strong></p>

  <p>Le <a href="https://bottin.rsn.quebec" style="color:#2b6cb0">bottin des membres du RSN</a> a beaucoup changé cet automne, et votre profil mérite d'en profiter. Selon nos dossiers, il est incomplet, ou vous ne nous avez pas encore dit si vous souhaitez qu'il soit visible publiquement.</p>

  <p>Compléter votre profil prend <strong>environ 5 minutes</strong>, et c'est ce qui permet aux autres membres de vous trouver.</p>

  <div style="text-align:center;margin:2rem 0">
    <a href="${magicUrl}" style="display:inline-block;padding:0.75rem 2rem;background:#2b6cb0;color:#ffffff;text-decoration:none;border-radius:0.5rem;font-weight:600;font-size:1rem">Compléter mon profil</a>
  </div>

  <p style="margin-bottom:0.5rem"><strong style="color:#1a365d">Ce qui est nouveau</strong></p>

  <div style="background:#ebf8ff;border-radius:0.5rem;padding:1rem 1.25rem;margin-bottom:1rem">
    <p style="margin:0 0 0.35rem 0"><strong>Les connexions.</strong> Une <a href="https://bottin.rsn.quebec/#v=connexions" style="color:#2b6cb0">carte des connexions</a> relie les membres qui partagent des thèmes de recherche ; un clic sur une personne affiche les profils les plus proches du sien. Vous n'y apparaissez que si vos thèmes et votre présentation sont remplis.</p>
    <img src="${baseUrl}/img/campagne-connexions.gif" alt="La vue Connexions du bottin : la carte des membres, puis le zoom sur une personne et ses liens" width="600" height="351" style="display:block;width:100%;max-width:600px;height:auto;margin:0.75rem auto 0.25rem auto;border-radius:0.5rem;border:1px solid #cbd5e0">
  </div>

  <div style="background:#ebf8ff;border-radius:0.5rem;padding:1rem 1.25rem;margin-bottom:1rem">
    <p style="margin:0 0 0.35rem 0"><strong>Un formulaire semi-automatisé.</strong> Le formulaire de mise à jour préremplit vos mots-clés à partir de vos publications : entrez votre identifiant ORCID, ou cherchez vos publications par votre nom, puis cochez les thèmes à garder.</p>
    <img src="${baseUrl}/img/campagne-suggestions.png" alt="Les thèmes proposés depuis un profil ORCID, à cocher" width="600" height="615" style="display:block;width:100%;max-width:600px;height:auto;margin:0.75rem auto 0.25rem auto;border-radius:0.5rem;border:1px solid #cbd5e0">
  </div>

  <div style="background:#ebf8ff;border-radius:0.5rem;padding:1rem 1.25rem;margin-bottom:1.25rem">
    <p style="margin:0"><strong>Une photo de profil.</strong> Facultative, cadrée en un clic, elle apparaît sur votre fiche et dans les connexions.</p>
  </div>

  <p style="margin-bottom:0.5rem"><strong style="color:#1a365d">Ce que nous vous demandons</strong></p>
  <ul style="margin-top:0;padding-left:1.25rem;color:#4a5568">
    <li>votre statut et un ou deux axes du RSN ;</li>
    <li>vos thèmes de recherche ou d'intérêt et une courte présentation ;</li>
    <li>votre réponse sur l'affichage public de votre profil ;</li>
    <li>si vous le souhaitez, une photo, votre ORCID et votre site web.</li>
  </ul>
  ...
  <p style="font-size:0.85rem;color:#718096">Ce lien est personnel et valide pendant 30 jours ; merci de ne pas le transférer. Il ne concerne pas le renouvellement de votre adhésion, qui fait l'objet d'un courriel distinct.</p>

  <p>Pour toute question : <a href="mailto:${CONTACT_EMAIL}" style="color:#2b6cb0">${CONTACT_EMAIL}</a></p>
  <p>Au plaisir,<br><strong>L'équipe du RSN</strong></p>
  ...
  <p>The <a href="https://bottin.rsn.quebec" style="color:#2b6cb0">RSN member directory</a> has changed a great deal this fall, and your profile deserves to benefit from it. According to our records, it is incomplete, or you have not yet told us whether you want it to be publicly visible.</p>

  <p>Completing your profile takes <strong>about 5 minutes</strong>, and it is what lets other members find you.</p>

  <div style="text-align:center;margin:2rem 0">
    <a href="${magicUrl}" style="display:inline-block;padding:0.75rem 2rem;background:#2b6cb0;color:#ffffff;text-decoration:none;border-radius:0.5rem;font-weight:600;font-size:1rem">Complete my profile</a>
  </div>

  <p style="margin-bottom:0.5rem"><strong style="color:#1a365d">What's new</strong></p>

  <div style="background:#ebf8ff;border-radius:0.5rem;padding:1rem 1.25rem;margin-bottom:1rem">
    <p style="margin:0 0 0.35rem 0"><strong>Connections.</strong> A <a href="https://bottin.rsn.quebec/#v=connexions&lang=en" style="color:#2b6cb0">connections map</a> links members who share research topics; clicking a person shows the profiles closest to theirs. You only appear on it if your topics and your presentation are filled in.</p>
    <img src="${baseUrl}/img/campagne-connexions.gif" alt="The directory Connections view: the member map, then a zoom on one person and their links" width="600" height="351" style="display:block;width:100%;max-width:600px;height:auto;margin:0.75rem auto 0.25rem auto;border-radius:0.5rem;border:1px solid #cbd5e0">
  </div>

  <div style="background:#ebf8ff;border-radius:0.5rem;padding:1rem 1.25rem;margin-bottom:1rem">
    <p style="margin:0 0 0.35rem 0"><strong>A semi-automated form.</strong> The update form pre-fills your keywords from your publications: enter your ORCID identifier, or search for your publications by name, then tick the topics to keep.</p>
    <img src="${baseUrl}/img/campagne-suggestions.png" alt="Topics suggested from an ORCID profile, to tick" width="600" height="615" style="display:block;width:100%;max-width:600px;height:auto;margin:0.75rem auto 0.25rem auto;border-radius:0.5rem;border:1px solid #cbd5e0">
  </div>

  <div style="background:#ebf8ff;border-radius:0.5rem;padding:1rem 1.25rem;margin-bottom:1.25rem">
    <p style="margin:0"><strong>A profile photo.</strong> Optional, cropped in one click, it appears on your card and in the connections.</p>
  </div>

  <p style="margin-bottom:0.5rem"><strong style="color:#1a365d">What we ask of you</strong></p>
  <ul style="margin-top:0;padding-left:1.25rem;color:#4a5568">
    <li>your status and one or two RSN axes;</li>
    <li>your research or interest topics and a short presentation;</li>
    <li>your answer about the public display of your profile;</li>
    <li>if you wish, a photo, your ORCID and your website.</li>
  </ul>
  ...
  <p style="font-size:0.85rem;color:#718096">This link is personal and valid for 30 days; please do not forward it. It is not about renewing your membership, which is handled in a separate email.</p>

  <p>For any questions: <a href="mailto:${CONTACT_EMAIL}" style="color:#2b6cb0">${CONTACT_EMAIL}</a></p>
  <p>Best regards,<br><strong>The RSN team</strong></p>
  `;
  await _send({ to, subject, html: emailWrapper(inner) });
}
